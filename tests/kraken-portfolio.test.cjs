...
const {krakenNumber,krakenMoney,krakenApy,renderKrakenPortfolio}=require('../static/kraken-portfolio.js');
test('formatting distinguishes missing values, zero and negative amounts',()=>{assert.equal(krakenMoney(null),'Unavailable');assert.equal(krakenMoney(0),'$0.00');assert.equal(krakenMoney(-1234),"-$1'234.00");assert.equal(krakenNumber(1.234567891),'1.23456789');assert.equal(krakenApy(3.125),'3.13%');assert.equal(krakenApy(null),'Unavailable');});
test('renderer uses text only and marks unpriced positions',()=>{
 ...
 assert.equal(nodes.get('krakenTotal').textContent,'Unavailable');assert.match(nodes.get('krakenCoverage').textContent,/<bad>/);const cells=nodes.get('krakenRows').children[0].children;assert.equal(cells[0].children[0].children[1].textContent,'<img>');assert.equal(cells[1].textContent,'Kraken');assert.equal(cells.length,7);assert.ok(cells[4].classList.values.includes('text-warning'));assert.ok(cells[5].classList.values.includes('text-warning'));
...
test('renderer adds CoinGecko icon and origin',()=>{
 ...
 try{renderKrakenPortfolio({total_value_usd:100,known_value_usd:100,complete:true,unpriced_assets:[],as_of:'2026-09-01T00:00:00Z',positions:[{asset:'BTC',origin:'Kraken',balance:1,apy:2.5,apy_source:'Kraken Earn estimate',price_usd:100,value_usd:100,price_pair:'XBTUSD',status:'priced',market_data:{coin_id:'bitcoin',image:'https://example.test/btc.png',status:'fresh'}}]});const cells=nodes.get('krakenRows').children[0].children;assert.equal(cells[0].children[0].children[0].src,'https://example.test/btc.png');assert.equal(cells[1].textContent,'Kraken');assert.equal(cells[3].textContent,'2.50%');assert.equal(cells.length,7);}finally{delete global.document;}
});
